  ...
  <div class="department-container row">
    <div class="q-pa-md">
      <div class="row justify-center q-gutter-sm">
        <div
          v-for="(project, ind) in getopenprojects"
          :key="project._id"
          :value="ind"
          ...
          <transition name="q-transition--scale">
            <q-card>
              <q-card-section>
                ...
                <q-btn @click="apply(ind)"> apply </q-btn>
              </q-card-section>
            </q-card>
          </transition>
        </div>
...
export default {
  ...
  async created() {
    ...
    try {
      this.projects = await allprojectservice.getprojectsbydept(
    ...
    getopenprojects() {
      return this.projects.filter((project) => {
        return (
          project.status &&
          project.number > 0 &&
          !this.rejected.includes(project._id)
        );
      });
    },
  },
  methods: {
    async apply(ind) {
      await projectservice
        .getaproject(this.projects[ind]._id)
        .then(async (response) => {
          await profservice
            .getprofbyid(response.professor)
            .then(async (res) => {
              res.notifs.push({
                studentid: this.student,
                projectid: this.projects[ind]._id,
              });
